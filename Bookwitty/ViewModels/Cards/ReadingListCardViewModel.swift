...
class ReadingListCardViewModel: CardViewModelProtocol {
  ...
  func values() -> (infoNode: Bool, postInfo: CardPostInfoNodeData?, content: (title: String?, description: String?, topComment: Comment?, comments: String?, tags: [String]?, relatedContent: (posts: [String], count: Int), statistics: (posts: Int?, relatedBooks: Int?, followers: Int?), wit: (is: Bool, count: Int, info: String?)), reported: Reported) {
    guard let resource = resource, let readingList = resource as? ReadingList else {
      return (false, nil, content: (nil, nil, nil, nil, nil, relatedContent: ([], 0), statistics: (nil, nil, nil), wit: (false, 0, nil)), .not)
    }
